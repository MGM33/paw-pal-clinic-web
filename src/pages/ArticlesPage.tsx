...
const ArticlesPage = () => {
  ...
  const filteredArticles = articlesData.filter((article) => {
    ...
    if (selectedCategory === 'cats') return article.id > 1000; // placeholder for future cat articles
    return false;
  });
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex justify-center gap-4 flex-wrap mb-10">
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-full border-2 transition-colors font-semibold ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" dir="rtl">
          {filteredArticles.length > 0 ? (
            ...
            <p className="text-center col-span-3 text-gray-500 text-lg">لا توجد مقالات متاحة لهذا التصنيف حالياً.</p>
          )}
        </div>
      </div>
    </section>
